...
<div class="warp">
    <!-- 添加 -->
          <div class="search_box">
            <el-button type="primary" @click="wealthGradeAdd">添加</el-button>
          </div>
          <!--表格-->
          <el-table
            :data="list"
            ...
            <el-table-column label="操作" min-width="15%" align="center">
              <template slot-scope="scope">
                <el-button
                  ...
                  size="mini"
                  type="info"
                  @click="wealthGradeEdit(scope.row.id)"
                  >编辑</el-button
                >
                <el-button
                  class="btn_operation"
                  size="mini"
                  type="danger"
                  @click="wealthDeleteGrade(scope.row.id)"
                  >删除</el-button
                >
              </template>
            </el-table-column>
...
import { wealthGradeList,wealthDeleteGrade } from "@/api/grade";
import Pagination from "@/components/Pagination";
import WealthGradeDetail from "@/components/grade/wealthGradeDetail";
export default {
props:[],
components: {
    Pagination,
...
methods:{
    ...
    wealthDeleteGrade(id) {
      ...
      this.$confirm("是否删除该等级?", "提示", {
        ...
        .then(() => {
          wealthDeleteGrade(this).then((res) => {
            this.$message({
              type: "success",
              message: "删除成功!",
            });
            this.wealthGradeList();
          });
        })
        .catch(() => {
          this.$message({
