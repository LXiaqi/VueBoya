...
Vue.use(VueDirectiveImagePreviewer,{zIndex: 9999,previewSize:6})
...
Vue.use(VueLazyload,{
  error: require('./assets/noAvatar.png')  //加载失败图片
})
...
if(process.env.NODE_ENV=='production') {
  ...
  // Axios.defaults.baseURL = 'http://47.114.175.22:15672/admin_api'
}
...
Axios.interceptors.request.use(config => {
  var token = sessionStorage.getItem('token');
  // console.log(token);
  if(config.url !== 'http://47.114.175.22:8081/login/index') {
